-- 2028_comment_notifications_enforce.sql
-- Purpose: ensure comment notifications are emitted from RPC and allowed by create_notification auth checks.

SET search_path = public, extensions;

-- Drop any stale 2-arg overload to avoid clients hitting an old definition.
DROP FUNCTION IF EXISTS public.create_comment_with_rate_limit(uuid, text);

-- Recreate 3-arg comment RPC with notification dispatch.
...
BEGIN
  ...
  -- Notify catch owner (skip self-comments, skip soft-deleted catches)
...
-- Recreate create_notification to clarify auth allowance for actor->recipient and keep dedupe scoped to likes/follows.
...
BEGIN
  ...
  -- Allow if requester is admin OR requester is the intended actor OR requester is the recipient (self-notify no-op)
  IF NOT v_is_admin THEN
