-- 2020_fix_notification_dedupe.sql
-- Purpose: Fix notification dedupe by replacing the prior index with a table constraint and upsert logic.
-- Safe to run on DBs where 2019_phase1c_notification_dedupe.sql already ran.

SET search_path = public, extensions;

-- 1) Clean up duplicate like/follow notifications (dedupe by user_id, actor_id, type)
...
-- 4) Recreate create_notification with upsert on the table constraint for like/follow notifications
